...
export function AppHeader() {
  ...
  return (
    <Box component='header' sx={{ display: 'flex', alignItems: 'center', gridGap: [4, 8] }}>
      <Link button href={linkToRoot ? WEB.ROUTES.ROOT : WEB.ROUTES.PHOTOS} sx={{ flex: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gridGap: 8, svg: { display: ['none', 'inherit'] } }}>
          ...
          <Typography sx={{ fontFamily: 'gelica, serif', fontWeight: 600, letterSpacing: 0.05 }} variant='h5'>
            Quiver Photos
          </Typography>
        </Box>
      </Link>
      <LogInOrContinue sx={{ paddingRight: [0, 2] }} />
      <AppUserBubble />
    </Box>
  );
}
